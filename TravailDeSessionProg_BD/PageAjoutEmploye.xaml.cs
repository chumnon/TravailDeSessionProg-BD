...
namespace TravailDeSessionProg_BD
{
    public sealed partial class PageAjoutEmploye : Page
    {
        ...
        private async void btAjouter_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ValidationEmploye.getInstance().isDateDeNaissanceValide(inDateDeNaissance.Date.ToString()) == 0)
            {
                ErrDateDeNaissance.Text = "L'employé est trop jeune pour travailler ici";
                valide = false;
            }
            else if (ValidationEmploye.getInstance().isDateDeNaissanceValide(inDateDeNaissance.Date.ToString()) == 1)
            {
            ...
            if (ValidationEmploye.getInstance().isDateEmbaucheValide(inDateEmbauche.Date.ToString()) == 0)
            {
                ErrDateEmbauche.Text = "La date d'embauche ne peut pas être dans le future";
                valide = false;
            }
            else if (ValidationEmploye.getInstance().isDateDeNaissanceValide(inDateEmbauche.Date.ToString()) == 1)
            {
            ...
            if (valide == true)
            {
                Employe unEmploye = new Employe
                {
                    ...
                    DateDeNaissance = inDateDeNaissance.Date.ToString().Substring(0, 10),
                    Email = inEmail.Text,
                    Adresse = inAdresse.Text,
                    DateEmbauche = inDateEmbauche.Date.ToString().Substring(0, 10),
